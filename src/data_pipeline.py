...
print(df.isnull().sum()[df.isnull().sum() > 0]) # Mostra solo colonne con null

# 2. Preprocessing base per Prolog
# Rimuoviamo eventuali spazi dai nomi delle colonne e rendiamo tutto minuscolo
# per rispettare la sintassi dei fatti Prolog (che richiede atomi in minuscolo)
df.columns = df.columns.str.lower().str.replace(' ', '_')

# Alcune variabili categoriche potrebbero avere spazi (es. "Software Engineer")
# Le trasformiamo sostituendo gli spazi con underscore
...
# 3. Generazione del file Prolog (.pl)
...
with open(kb_path, 'w') as f:
    ...
    for index, row in df.iterrows():
        # Estraiamo le colonne chiave per la nostra KB
        p_id = row['person_id']
        eta = row['age']
        genere = row['gender']
        ...
        rischio = row['sleep_disorder_risk'] # Il nostro target
